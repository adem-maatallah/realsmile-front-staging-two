...
import { FaMapMarkerAlt, FaSearchLocation, FaGlobe, FaPhoneAlt, FaEnvelope, FaInfoCircle, FaSortAmountUpAlt } from 'react-icons/fa'; // Added FaSortAmountUpAlt for list icon
...
const mapContainerStyle = {
    width: '100%', // Changed to 100% for full width in its container
    height: '450px', // Slightly increased height for better view
    borderRadius: '1rem',
...
export default function FindDoctorsPage() {
    ...
    const [closestDoctorsList, setClosestDoctorsList] = useState<DoctorLocation[]>([]); // New state for the list
    ...
    const findClosestDoctor = useCallback(() => {
        ...
        setClosestDoctorsList([]); // Clear previous list when searching again
        ...
        if (doctorsWithDistance.length > 0) {
            setClosestDoctor(doctorsWithDistance[0]); // Set the single closest doctor for map highlight
            setClosestDoctorsList(doctorsWithDistance); // Set the full sorted list
            toast.success(`Closest doctor found: ${doctorsWithDistance[0].user_name || 'Doctor'} (${doctorsWithDistance[0].distance?.toFixed(2)} km away)`);

            // Adjust map to fit user location and the top few closest doctors
            if (mapInstanceRef.current && userLocation) {
                const map = mapInstanceRef.current;
                const newBounds = new window.google.maps.LatLngBounds();
                newBounds.extend({ lat: userLocation.latitude, lng: userLocation.longitude });

                // Add top 5 closest doctors to bounds (or fewer if less available)
            ...
            setActiveInfoWindow(doctorsWithDistance[0]); // Open info window for the closest one
        } else {
    ...
    return (
        <div className="min-h-screen p-4 sm:p-8 font-inter flex flex-col items-center bg-gray-50"> {/* Changed background color */}
            <div className="max-w-7xl w-full mx-auto bg-white rounded-3xl shadow-xl p-6 sm:p-10 border-px border-gray-200">
                ...
                {/* Main Action Section: Find Closest Doctor Button and User Location Info */}
                <div className="flex flex-col md:flex-row justify-center items-center gap-6 mb-8">
                    ...
                    <Button
                        ...
                        style={{
                            ...
                            '--tw-ring-color': DEFAULT_PRESET_COLORS.light, // Custom property for ring color
                    ...
                    >
                        {isFindingClosestDoctor ? (
                            ...
                            <>
                                <FaSearchLocation className="mr-2 text-xl" /> Find Closest Doctor
                            </>
                        )}
                    </Button>
                    ...
                    {userLocation && (
                        ...
                        >
                            ...
                            <div>
                                <h2 className="font-bold text-base mb-1">Your Location:</h2>
                                {loadingUserLocation ? (
                                    ...
                                    <>
                                        ...
                                        {userLocation.permissionStatus === 'denied' && (
                                            <p className="text-xs mt-1 text-red-600 flex items-center">
                                                <FaInfoCircle className="mr-1" /> Location denied. Enable in settings.
                                            </p>
                                        )}
                                        {userLocation.permissionStatus === 'prompt' && (
                                            <p className="text-xs mt-1 text-blue-700 flex items-center">
                                                <FaInfoCircle className="mr-1" /> Grant location permission when prompted.
                                            </p>
                                        )}
                                    </>
                                )}
                ...
                {/* Closest Doctors List Section */}
                {closestDoctorsList.length > 0 && (
                    <>
                        <hr className="my-8 border-gray-200" /> {/* Separator */}
                        ...
                        >
                            <h2 className="font-bold text-xl mb-4 flex items-center">
                                <FaSortAmountUpAlt className="mr-3 text-2xl" style={{ color: DEFAULT_PRESET_COLORS.default }} /> Top Closest Doctors:
                            </h2>
                            <ul className="space-y-3">
                                {closestDoctorsList.slice(0, 5).map((doc, index) => ( // Display top 5 in the list
                                    <li key={doc.id} className="p-3 rounded-lg flex items-center border border-gray-200 bg-white shadow-sm hover:shadow-md transition-shadow">
                                        {doc.profile_pic && (
                                            <Image
                                                src={doc.profile_pic}
                                                alt={doc.user_name || 'Doctor Profile'}
                                                width={50}
                                                height={50}
                                        ...
                                        <div className="flex-grow">
                                            <p className="font-semibold text-base" style={{ color: DEFAULT_PRESET_COLORS.dark }}>
                                                {index + 1}. {doc.user_name || `${doc.first_name || ''} ${doc.last_name || ''}`.trim() || 'Doctor'}
                                                {doc.distance !== undefined && <span className="ml-2 text-gray-500 text-sm">({doc.distance.toFixed(2)} km)</span>}
                                            </p>
                                            {doc.speciality && <p className="text-sm text-gray-600">Speciality: {doc.speciality}</p>}
                                        ...
                                        <Link
                                            href={
                                                routes.doctors?.details // Check if 'doctors' and 'details' exist
                                                    ? (typeof routes.doctors.details === 'function' // Check if 'details' is a function
                                                        ? routes.doctors.details(doc.id) // If it's a function, call it
                                                        : `${routes.doctors.details}/${doc.id}` // If it's a string, concatenate
                                                    )
                                                    : `/doctors/${doc.id}` // Fallback if 'routes.doctors.details' is undefined
                                            }
                                            passHref
                                        >
                                            <Button
                            ...
                            {closestDoctorsList.length > 5 && (
                                <p className="text-center text-sm text-gray-600 mt-4">
                                    Showing top 5 doctors. More available on the map.
                                </p>
                            )}
                        </div>
                    </>
