...
export const DEFAULT_PRESET_COLORS = {
  ...
  default: '#d39424', // <-- MISE À JOUR : Votre nouveau jaune spécifique
  dark: '#a16207', // Jaune 800
  foreground: '#ffffff',
};
...
export default function OtherPagesLayout({
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      // Vérification initiale au cas où la page se charge déjà défilée
      handleScroll();
  ...
  return (
    ...
    >
      {/* En-tête : Fixé, animé pour apparaître au défilement */}
      <header
        className={cn(
          ...
          "px-6 py-5 md:px-10 md:py-6 lg:px-12 lg:py-7 backdrop-blur-sm" // Ajout d'un flou d'arrière-plan pour un aspect moderne
        )}
        style={{
          backgroundColor: showHeader ? 'rgba(255, 255, 255, 0.95)' : 'transparent', // Rendre l'en-tête translucide lorsqu'il est visible
        }}
      >
          ...
          <nav className="flex items-center gap-4">
            {shouldShowSignInButton && (
              <Button
                variant="solid"
                size="sm"
                className="h-10 px-5 text-base font-semibold rounded-full md:h-11 md:px-6 md:text-lg focus:ring-2 focus:ring-offset-2" // Ajout d'un anneau de focus
                style={{
                  ...
                  boxShadow: `0 2px 4px ${DEFAULT_PRESET_COLORS.light}` // Ombre subtile pour la profondeur
                }}
                ...
                onClick={() => push(routes.auth.signIn1)} // Changé à routes.auth.signIn pour la cohérence
              >
                <FaSignInAlt className="mr-2 h-4 w-4" /> Se connecter
              </Button>
            )}
          </nav>
        </div>
      ...
      {/* Zone de contenu principal avec dégradé de fond et motif */}
      <div
        ...
        style={{
          background: `linear-gradient(135deg, ${DEFAULT_PRESET_COLORS.lighter} 0%, ${DEFAULT_PRESET_COLORS.light} 100%)`, // Dégradé ajusté pour une transition plus douce
          '--header-height': showHeader ? '100px' : '0px',
          paddingTop: showHeader ? 'calc(100px + 2rem)' : '2rem' // Ajouter un padding-top lorsque l'en-tête est visible pour éviter le chevauchement du contenu
      ...
      >
        ...
        <div className="relative z-20 max-w-4xl mx-auto px-6 py-12">
          ...
          >
            Votre Santé, <span style={{ color: DEFAULT_PRESET_COLORS.lighter }}>Simplifiée.</span>
          </h1>
          <p
          ...
          >
            Découvrez et connectez-vous avec les meilleurs professionnels de la santé près de chez vous avec une facilité inégalée.
            Des soins experts à portée de clic.
          </p>
          <Button
            variant="solid"
            size="lg"
            className="px-8 py-3 text-xl font-bold rounded-full shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 focus:ring-2 focus:ring-offset-2" // Ajout d'un anneau de focus
            style={{
      ...
      >
        <div className="max-w-[1520px] mx-auto flex flex-col sm:flex-row justify-between items-center">
          <p>© {new Date().getFullYear()} {siteConfig.title}. Tous droits réservés.</p>
