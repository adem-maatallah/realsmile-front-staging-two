...
export default function StlUploadZone({
                                          label,
                                          name,
                                          className,
                                          getValues,
    ...
    const uploadedItems = Array.isArray(getValues(name)) ? getValues(name) : [];
            ...
            {(!isEmpty(uploadedItems) || !isEmpty(notUploadedItems)) && (
                <div className="mt-5 grid grid-cols-2 gap-4 sm:grid-cols-[repeat(auto-fit,_minmax(140px,_1fr))]">
                    ...
                    {notUploadedItems.map((file: any, index: number) => (
                        <div key={index} className={cn('relative')}>
                            <figure className="group relative h-full rounded-md bg-gray-50">
                                <MediaPreview name={file.name} url={file.preview}/>
                                {isUploading ? (
                                    <div
                                        className="absolute inset-0 z-50 grid place-content-center rounded-md bg-gray-800/50">
                                        <LoadingSpinner/>
                                    </div>
                                ) : (
                                    <button
...
function MediaPreview({ name, url }: { name: string; url: string }) {
    return endsWith(name, '.stl') ? (
        <object data={url} type="application/vnd.ms-pkistl" width="100%" height="100%">
            <StlFileDisplay title={name} url={url} />
        </object>
    ) : (
        <Image
            fill
