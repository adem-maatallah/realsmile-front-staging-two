...
const fetchCaseFiles = async (caseId: string, token: any): Promise<any> => {
  const apiUrl = process.env.NEXT_PUBLIC_API_URL;
  const endpoint = `/laboratories/laboratoryCaseFiles/${encodeURIComponent(caseId)}`;
  ...
  const caseFilesData: any = await response.data;
  return caseFilesData;
};

export { fetchCaseFiles };
