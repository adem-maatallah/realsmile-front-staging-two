...
export function usePatientsTable<T extends AnyObject>(
  initialData: T[],
  ...
  function searchedData() {
    ...
    // Only filter on the "patient" column
    return sortedData.filter((item) => {
      const patientName = item.patient?.name?.toLowerCase();
      const patientPhone = item.patient?.phone?.toLowerCase();

      // Search only in the patient's name and phone fields
      return (
        (patientName && patientName.includes(searchTermLower)) ||
        (patientPhone && patientPhone.includes(searchTermLower))
      );
    });
  }
