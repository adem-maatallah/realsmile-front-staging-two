  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (ref.current && !ref.current.contains(event.target)) {
        setIsOpen(false); // Only close if clicked outside
      }
    };
  ...
  return (
    <div ref={ref}>
      <div className="relative mt-1">
        <button
          type="button"
          className="relative w-full cursor-default rounded-md border border-gray-300 py-2 pl-3 pr-10 text-left shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 sm:text-sm"
        ...
        <AnimatePresence>
          {open && (
            <motion.ul
              ...
              className="position-absolute z-1050 mt-1 max-h-80 w-full rounded-md bg-white text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm" // Adjust z-index higher than any other element
              tabIndex={-1}
              role="listbox"
